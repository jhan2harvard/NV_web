...
window.onload = function what(){
    let station_name = "Boston Logan IntL Arpt"
    // document.getElementById("title_location").innerHTML = "Regional NV hour - " + station_name;
...
function chart1() {
    ...
    // d3.json("../static/data/Station-NV.json", (row) => {
    //     return row
    // }).then( data => {
    //     dataset = data;
    ...
    // d3.json("../static/data/Station-NV.json", function (row) {
    //     return row
    // }).then(function (data) {
    //     dataset = data;
    //     draw(dataset, station_name);
    ...
    // d3.json("../data/Zip_data+station.json", function(dd) {
    //   console.log(dd);
    //   dataset = data;
    //   draw(dataset, station_name);
    // });
    //
      $.getJSON("../data/Zip_data+station.json", function(data) {
          console.log("Sample_outcome worked!");
          console.log(data);
      });

    function draw(data, location) {
        // 지정된 station의 NV hour 데이터 dataset variable로 저장
        let dataset = data[location]["RNV_hour"];
        ...
        svg.append("g")
            ...
            .on("click", (d, i) => {
                let month_sub = "NV hour on " + months[i-1]
            ...
            .text(function(d) {
                return "Month: " + months[d-1]
                    ...
                    + "\n" + "NV hours: " + NV_month(d)
                    + "\n" + "NV percent: " + Math.round(NV_month(d) / (hours[d-1][1] - hours[d-1][0] + 1) * 100) + "%";
            });
    }
}
...
function chart2(i, k) {
    ...
    function draw(data, location, i, k) {
        ...
        let dataset = data[location]["RNV_hour"].slice(start, end);
...
function chart3(i) {
    ...
    function draw(data, location, i) {
        ...
        let colorset = data[location]["RNV_hour"].slice(i, i + 24);
